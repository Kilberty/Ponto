...
private function getPontosFormatados($funcionarioId, $inicio, $fim, $perPage)
{
     ...
     public function pdfObra(Request $request)
     {
        $dados = $request->query();
        $construction = Construction::findOrFail($dados['obra']);
        $dataInicio = Carbon::createFromFormat('Y-m-d', $dados['inicio']);
        $dataFim = Carbon::createFromFormat('Y-m-d', $dados['fim']);
        $inicioFormatado = $dataInicio->format('d-m-Y');
        $fimFormatado = $dataFim->format('d-m-Y');
        
        
        $funcionarios = ConstructionEmployee::where('construcao_id', $dados['obra'])
                        ->with('employee')
                        ->get();
        

        // ** Adição: Verifica se há funcionários para a obra **
        if ($funcionarios->isEmpty()) {
            // Retorna uma resposta JSON amigável para o frontend
            return response()->json(['message' => 'Nenhum relatório encontrado para a obra selecionada.']);
        }
        $zipFileName = "obra_{$construction->nome}_{$inicioFormatado}-{$fimFormatado}.zip";
        ...
        if ($zip->open($zipPath, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
            foreach ($funcionarios as $funcionario) {
                $empresa = Business::findOrFail($funcionario->employee->empresa_id);
                $pontos = $this->getPontosFormatados(
                ...
                $pdfFileName = "{$funcionario->employee->nome}____{$inicioFormatado}----{$fimFormatado}.pdf";
                // Adiciona ao ZIP diretamente sem salvar em disco
